...
module Onboardable
  # The ListBuilder class constructs and manages an onboarding step list, adding steps and building the final list.
  class ListBuilder
    ...
    # @return [List] A new List object initialized with the steps and the specified current step.
    # @raise [EmptyStepsError] if no steps have been added to the builder.
    def build!(current_step_name)
      List.new(convert_to_steps!, convert_to_step!(current_step_name || current_step.name))
    end
